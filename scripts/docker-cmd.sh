...
then
	...
	crond -s /var/spool/cron/crontabs -b -l info -L /var/log/cron/cron.log "$@" && tail -f /var/log/cron/cron.log
else
	# crond running in foreground. log files can be retrive from /var/log/cron mount point
	crond -s /var/spool/cron/crontabs -f -l info -L /var/log/cron/cron.log "$@"
fi
